  ...
  export default function AddLocation({ navigation }) {
    ...
    const fetchLocations = async () => {
      try {
        const response = await axios.get(`http://192.168.1.16:5000/api/getLocations/${streetId}`);
    ...
    const handleAddLocation = async () => {
      try {
        await axios.post('http://192.168.1.16:5000/api/addLocation', { latitude, longitude, rueId: streetId });
    ...
    const handleDeleteLocation = async (locationId) => {
      try {
        await axios.delete(`http://192.168.1.16:5000/api/deleteLocation/${locationId}`);
        fetchLocations();
      } catch (error) {
